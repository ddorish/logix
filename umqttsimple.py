...
class MQTTClient:
    ping_rate = 2000   # Send a ping to server if no activity for that mSec

    def __init__(self, client_id, server, port=None, user=None, password=None, keepalive=0,
    ...
    def subscribe(self, topic, qos=0, max_retries=0):
        ...
        num_retries = 0
        while max_retries == 0 or max_retries > num_retries:
            num_retries += 1
            op = self.wait_msg()
            if op == 0x90:
                resp = self.sock.read(4)
